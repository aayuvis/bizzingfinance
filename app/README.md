...
| `src/objectives-more.js` | The other five strands — EARN, KEEP, GROW, OWE, GUARD — concatenated into the graph in `objectives.js` so `validate()` sees them. Thirty-six rungs; a transfer surface is never the surface it was taught on, and the validator enforces that. |
| `src/sources.js` | Rule six with a mechanism: every figure is either a dial of this town, said plainly, or a real figure with a citation. `test/sources.mjs` fails a card that states a real-world figure without one. |
| `src/dailypuzzle.js` | Today's till — one puzzle a day, the same one in every house, deterministic from the date. Three tries, one wage into the same wallet, a share that carries no answer, and nowhere in the record to keep a streak. |
| `src/placement.js` | The maths check. Twelve rungs of the number spine, stopped at two wrong in a row, writing a **ceiling** the town obeys — never a score, never shown to the child, never in a report. It replaces `ledger.js`'s band guess. |
| `src/backup.js` | The household out to a file and back, plus the **allow-list** a sync would have to obey. A child's name and any recorded voice are not on it, so the promise is structural. Consent is its own switch and withdrawing deletes. |
| `src/answers.js` | The week's question, answered out loud and kept. The microphone opens only on a real tap and the track stops the instant it ends; the audio goes to IndexedDB through the `Store` seam and never joins the household blob. |
| `src/walks-gen.js` | Generated. Five painted panoramas, one per world, each carrying the **measured** height of its own road so the Atlas stops stand on it. |
| `src/daily.js` | The dailies on Home — Do one (a real-world deed, kept), Carry one (a glossary word), Ask at home (a weekly question about a grown-up's own past), a tip from a card already read. Rotates from the day, never chance. |
| `src/quiz.js` | Six mixed questions from one chapter, two doors: **test out** of a level-locked chapter (opens it, cards stay unread) and a **checkpoint** after a finished one (score on the rail). |
...
## What the siblings taught it

[docs/11-made-whole.md](../docs/11-made-whole.md) is the feature-by-feature diagnosis against
Bizzing Bee and Bizzing India — every row Have / Built / Own way / Refused, with the reason.
Read it before adding a feature "because Bee has it": three of Bee's are refused here on
purpose (coins, packs, streak rewards), and the reasons are the app's own rules.
